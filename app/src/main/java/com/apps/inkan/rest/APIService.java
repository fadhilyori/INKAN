...
public class APIService {
    ...
    public static String BASE_URL = "http://192.168.1.8/inkan/backend/web/api/";
...
//        Call<UserModel> getUser(@Header("Authorization") String password_hash);
        ...
        Call<ArtikelModel> getArtikel(@Header("Authorization") String id);
    ...
    public interface PostService{
        @POST("user/login")
        @FormUrlEncoded
        Call<LoginModel> postLogin(@Field("username") String username, @Field("password_hash") String password_hash);

        @POST("user/register")
        @FormUrlEncoded
        Call<UserModel> postRegister(@Field("username") String username, @Field("email") String email, @Field("password_hash") String password);
        ...
        Call<SaleModel> postSale (
                ...
                @Field("gambar") String gambar, @Field("keterangan") String keterangan, @Field("harga") int harga, @Field("fish_kategori_id") int fish_kategori_id, @Field("fish_id") int fish_id
        );

        @POST("user/updateuser")
        @FormUrlEncoded
        Call<UserModel> postUser (@Field("username") String username, @Field("email") String email, @Field("phone_telp") String harga, @Field("address") String alamat
        );

        @POST("artikel/createartikel")
        @FormUrlEncoded
        Call<ArtikelModel> postAtikel(
                @Header("Authorization") String auth,
